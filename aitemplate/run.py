...
model, _, preprocess = open_clip.create_model_and_transforms(
    "ViT-SO400M-14-SigLIP-384", pretrained="webli", precision="fp16", device="cuda"
)
model.eval()
...
def compile_vit(
    ...
    ait_model = VisionTransformer(batch_size=batch_size, seq_len=seq_len, **config)
    ait_model.name_parameter_tensor()
    print(ait_model)
    inputs_ait = Tensor(
        [batch_size, config["img_size"], config["img_size"], config["in_chans"]],
        name="input0",
        is_input=True,
    )
