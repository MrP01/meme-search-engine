...
def rationalize_coords_format1(
    image_w,
    image_h,
    center_x_fraction,
    center_y_fraction,
...
async def scan_image_chunk(sess, image):
    ...
    try:
        ...
        text_regions = [
            rationalize_coords_format1(image.width, image.height, *x[1])
            for x in root[2][3][0]
            if x[11].startswith("text:")
        ]
...
def chunk_image(image: Image):
    ...
    if image.width > MAX_SCAN_DIM:
        image = image.resize(
            (MAX_SCAN_DIM, round(image.height * (image.width / MAX_SCAN_DIM))),
            Image.LANCZOS,
        )
