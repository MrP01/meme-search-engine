...
use prometheus::{
  register_int_counter, register_int_counter_vec, register_int_gauge, Encoder, IntCounter, IntCounterVec, IntGauge,
};
use reqwest::Client;
...
use crate::common::{
  decode_fp16_buffer, get_backend_config, query_clip_server, resize_for_embed, EmbeddingRequest, EmbeddingVector,
  InferenceServerConfig, QueryRequest, QueryResult,
};
...
lazy_static! {
  ...
  static ref VIDEOS_LOADED_ERROR_COUNTER: IntCounter =
    register_int_counter!("mse_video_load_errors", "video load fails by ingest process").unwrap();
  ...
  static ref THUMBNAILS_GENERATED_COUNTER: IntCounterVec =
    register_int_counter_vec!("mse_thumbnail_outputs", "thumbnails produced by ingest process", &["output_format"])
      .unwrap();
...
fn generate_thumbnail_filename(filename: &Filename, format_name: &str, format_config: &ImageFormatConfig) -> String {
  format!("{}{}.{}", generate_filename_hash(filename), format_name, format_config.extension)
}
  ...
  for (i, vector) in result.into_iter().enumerate() {
    ...
    match &batch[i].filename {
      Filename::VideoFrame(container, _) => {
        video_embed_times.write().await.insert(container.clone(), timestamp());
      }
      _ => (),
    }
    sqlx::query!(
...
async fn load_image(
  ...
  let image: Result<Arc<DynamicImage>> =
    tokio::task::block_in_place(|| Ok(Arc::new(ImageReader::open(&path)?.with_guessed_format()?.decode()?)));
...
async fn generate_thumbnail(
  ...
  let generated_formats = tokio::task::spawn_blocking(move || {
    ...
    for (format_name, format_config) in &*formats {
      ...
      let resized = if format_config.target_filesize != 0 {
        ...
        loop {
          let quality = (lb + ub) / 2;
          let thumbnail = rgb.resize(format_config.target_width.min(rgb.width()), u32::MAX, FilterType::Lanczos3);
          ...
          match format_config.format {
            ImageFormat::Avif => {
              thumbnail.write_with_encoder(avif::AvifEncoder::new_with_speed_quality(&mut csr, 4, quality))
            }
        ...
        match format_config.format {
          ImageFormat::Avif => {
            thumbnail.write_with_encoder(avif::AvifEncoder::new_with_speed_quality(&mut csr, 4, format_config.quality))
          }
          ImageFormat::Jpeg => {
            thumbnail.write_with_encoder(jpeg::JpegEncoder::new_with_quality(&mut csr, format_config.quality))
          }
      ...
      if resized.len() < image.original_filesize.unwrap_or(usize::MAX) {
        ...
        let thumbnail_path = Path::new(&config.service.thumbs_path).join(generate_thumbnail_filename(
          &image.filename,
          format_name,
          format_config,
        ));
  ...
  match filename {
    Filename::VideoFrame(container, _) => {
      video_thumb_times.write().await.insert(container.clone(), timestamp());
    }
    _ => (),
  }
  sqlx::query!(
...
async fn ingest_files(config: Arc<WConfig>) -> Result<()> {
  ...
  for (container_filename, metadata) in video_meta.iter() {
    ...
    sqlx::query!(
      "INSERT OR REPLACE INTO files (filename, embedding_time, thumbnail_time, metadata) VALUES (?, ?, ?, ?)",
      container_filename,
      embed_time,
      thumb_time,
      metadata
...
async fn handle_request(
  ...
  let embedding = common::get_total_embedding(
    &req.terms,
    &config.backend,
    |batch, (config, client)| async move { query_clip_server(&client, &config.service.clip_server, "", batch).await },
