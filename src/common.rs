...
pub async fn resize_for_embed<T: Borrow<DynamicImage> + Send + 'static>(
    ...
    let resized =
        tokio::task::spawn_blocking(move || resize_for_embed_sync(&config, image)).await??;
    Ok(resized)
}
...
pub async fn query_clip_server<I, O>(
    client: &Client,
    base_url: &str,
    path: &str,
    data: I,
...
>(
    ...
    for term in terms {
        if let Some(image) = &term.image {
            TERMS_COUNTER
                .get_metric_with_label_values(&["image"])
                .unwrap()
                .inc();
        ...
        if let Some(text) = &term.text {
            TERMS_COUNTER
                .get_metric_with_label_values(&["text"])
                .unwrap()
                .inc();
