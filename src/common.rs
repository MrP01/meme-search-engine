...
pub fn resize_for_embed_sync<T: Borrow<DynamicImage> + Send + 'static>(
  ...
  let opts = ResizeOptions::default().resize_alg(ResizeAlg::Convolution(
    if original_width > config.image_size.0 && original_height > config.image_size.1 {
  ...
  RESIZER
    .with_borrow_mut(|resizer| resizer.resize(&src_rgb, &mut dst_image, Some(&opts)))
    .context("resize failure")?;

  let mut buf = Vec::new();
  let mut csr = Cursor::new(&mut buf);
  BmpEncoder::new(&mut csr).write_image(
    dst_image.buffer(),
    config.image_size.0,
    config.image_size.1,
...
pub async fn resize_for_embed<T: Borrow<DynamicImage> + Send + 'static>(
  config: InferenceServerConfig,
  image: T,
) -> Result<Vec<u8>> {
...
pub async fn query_clip_server<I, O>(client: &Client, base_url: &str, path: &str, data: I) -> Result<O>
where
  I: Serialize,
...
pub async fn get_total_embedding<
  A: Future<Output = Result<Vec<Vec<u8>>>>,
  B: Future<Output = Result<serde_bytes::ByteBuf>>,
  S: Clone,
  T: Clone,
  F: Fn(EmbeddingRequest, S) -> A,
  G: Fn(Vec<u8>, T) -> B,
>(
  terms: &Vec<QueryTerm>,
  ic: &InferenceServerConfig,
  query_server: F,
  resize_image: G,
  predefined_embeddings: &HashMap<String, ArrayBase<ndarray::OwnedRepr<f32>, ndarray::Dim<[usize; 1]>>>,
  image_state: T,
  query_state: S,
) -> Result<Vec<f32>> {
  ...
  for term in terms {
    ...
    if let Some(embedding) = &term.embedding {
      TERMS_COUNTER
        .get_metric_with_label_values(&["embedding"])
        .unwrap()
        .inc();
  ...
  if !image_batch.is_empty() {
    batches.push((EmbeddingRequest::Images { images: image_batch }, image_weights));
  }
  if !text_batch.is_empty() {
    batches.push((EmbeddingRequest::Text { text: text_batch }, text_weights));
  }
