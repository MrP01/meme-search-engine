...
use common::{
    IndexHeader, PackedIndexEntry, ProcessedEntry, ShardHeader, ShardInputHeader, ShardedRecord,
};
...
struct CLIArguments {
    #[argh(
        option,
        short = 's',
        description = "randomly select fraction of records"
    )]
    sample: Option<f32>,
    #[argh(
        switch,
        short = 'p',
        description = "print basic information for records"
    )]
    print_records: bool,
    ...
    #[argh(
        option,
        short = 'E',
        description = "x:y[:f] - load embedding named x from file y, discard record if dot product >= filter threshold f"
    )]
    embedding: Vec<String>,
    #[argh(
        option,
        short = 'H',
        description = "path for histograms of dot with embeddings"
    )]
    ...
    #[argh(
        option,
        short = 'C',
        description = "split input into shards using these centroids"
    )]
    ...
    #[argh(
        option,
        short = 'f',
        description = "k-means balance fudge factor",
        default = "0.2"
    )]
    balance_fudge: f64,
    ...
    #[argh(
        option,
        description = "postfilter by embedding (late discard if dot product above threshold)"
    )]
...
impl Histogram {
    ...
    fn buckets(&self) -> Vec<(f32, u64)> {
        ...
        self.buckets
            .iter()
            .enumerate()
            .map(|(i, x)| (self.min + (i as f32) * step, *x))
            .collect()
    }
}
...
fn reader_thread(paths: &Vec<String>, tx: SyncSender<ProcessedEntry>) -> Result<()> {
    for path in paths {
        ...
        loop {
            ...
            match res {
                Ok(x) => tx.send(x)?,
                Err(DecodeError::InvalidDataRead(x)) | Err(DecodeError::InvalidMarkerRead(x))
...
fn main() -> Result<()> {
    ...
    for x in args.embedding {
        ...
        let (path, threshold) = if let Some((path, threshold)) = snd.split_once(':') {
            (
                path,
                Some(threshold.parse::<f32>().context("parse threshold")?),
            )
        } else {
            (snd, None)
        };
        ...
        embeddings.push((
            name.to_string(),
            common::decode_fp16_buffer(&blob),
            Histogram::new(-1.0, 1.0, 512),
            threshold,
            post_threshold,
        ));
    }

    let postfilter_scorer = args
        .postfilter_scorer
        .iter()
        .map(|x| {
            let (id, snd) = x
                .split_once(':')
                .context("invalid postfilter scorer argument")?;
            Ok((
                id.parse::<usize>()
                    .context("invalid postfilter scorer id")?,
                snd.parse::<f32>()
                    .context("parse postfilter scorer threshold")?,
            ))
        })
    ...
    let pq_codec = if let Some(pq_codec) = args.pq_codec {
        ...
        let pq_codec: ProductQuantizer =
            rmp_serde::from_read(&data[..]).context("decode pq codec")?;
        Some(pq_codec)
    } else {
        None
    };
    ...
    let mut shards_out = if let (Some(shards_dir), Some(centroids)) =
        (&args.shards_dir, &args.centroids)
    {
        ...
        for i in 0..(centroids_data.len() / (D_EMB as usize)) {
            let centroid =
                centroids_data[i * (D_EMB as usize)..(i + 1) * (D_EMB as usize)].to_vec();
            let mut file =
                fs::File::create(PathBuf::from(shards_dir).join(format!("{}.shard.msgpack", i)))
    ...
    let (mut read_out_vertices, shard_specs) =
        if let (Some(shards_dir), Some(_index_output)) = (&args.shards_dir, &args.index_output) {
            ...
            for file in fs::read_dir(shards_dir)? {
                ...
                if snd == "shard-header.msgpack" {
                    let header: ShardHeader =
                        rmp_serde::from_read(BufReader::new(fs::File::open(path)?))?;
                    if original_ids_to_shards.len() < (header.max as usize + 1) {
                        // probably somewhat inefficient, oh well
                        original_ids_to_shards
                            .resize(header.max as usize + 1, [EMPTY_LOOKUP; SHARD_SPILL]);
                    }
                    ...
                    shard_specs.push((
                        header.centroid.clone(),
                        header.mapping[header.medioid as usize],
                    ));
    ...
    let mut index_output_file = if let Some(index_output) = &args.index_output {
        let main_output = BufWriter::new(
            fs::File::create(PathBuf::from(index_output).join("index.bin"))
                .context("create index file")?,
        );
        let pq_codes = BufWriter::new(
            fs::File::create(PathBuf::from(index_output).join("index.pq-codes.bin"))
                .context("create index file")?,
        );
        let descriptor_codes = BufWriter::new(
            fs::File::create(PathBuf::from(index_output).join("index.descriptor-codes.bin"))
                .context("create index file")?,
        );
    ...
    let mut output_file = args
        .output_embeddings
        .map(|x| fs::File::create(x).context("create output file"))
        .transpose()?;
    ...
    let initial_filter = |x: ProcessedEntry| {
        ...
        if args.print_records {
            println!(
                ...
                timestamp, x.title, x.subreddit, x.id, x.metadata.final_url
            );
        }
        ...
        if args.print_embeddings {
            println!(
                "https://mse.osmarks.net/?e={}",
                base64::engine::general_purpose::URL_SAFE.encode(&x.embedding)
            );
        }
    ...
    for batch in &rx.iter().filter_map(initial_filter).chunks(BATCH_SIZE) {
        ...
        if let (Some(read_out_vertices), Some(index_output_file)) =
            (&mut read_out_vertices, &mut index_output_file)
        {
            let quantizer = pq_codec
                .as_ref()
                .context("PQ codec needed to output index")?;
            ...
            let score_model = score_model
                .as_ref()
                .context("score model needed to output index")?;
            let cdfs = cdfs
                .as_ref()
                .context("score model CDFs needed to output index")?;
            ...
            for (i, (x, _embedding, mut postfilter)) in batch.into_iter().enumerate() {
                ...
                let mut entry_scores = scores
                    [(i * score_model.output_channels)..((i + 1) * score_model.output_channels)]
                    .to_vec();
                ...
                for (index, score) in entry_scores.iter().enumerate() {
                    ...
                    let cdf_bucket: u8 =
                        match cdfs[index].binary_search_by(|x| x.partial_cmp(score).unwrap()) {
    ...
    if args.print_aggregates {
        println!(
            "earliest={} latest={} count={} read={} deduped={} postfiltered={}",
            earliest_timestamp, latest_timestamp, count, i, deduped_count, postfilter_count
        );
    }
