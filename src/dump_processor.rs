...
use common::{IndexHeader, PackedIndexEntry, ProcessedEntry, ShardHeader, ShardInputHeader, ShardedRecord};
...
fn main() -> Result<()> {
  ...
  let postfilter_scorer = args
    ...
    .map(|x| {
      ...
      Ok((
        id.parse::<usize>().context("invalid postfilter scorer id")?,
        snd.parse::<f32>().context("parse postfilter scorer threshold")?,
      ))
    })
  ...
  let mut shards_out = if let (Some(shards_dir), Some(centroids)) = (&args.shards_dir, &args.centroids) {
    ...
    for i in 0..(centroids_data.len() / (D_EMB as usize)) {
      ...
      let mut file = fs::File::create(PathBuf::from(shards_dir).join(format!("{}.shard.msgpack", i)))
  ...
  let mut index_output_file = if let Some(index_output) = &args.index_output {
    let main_output =
      BufWriter::new(fs::File::create(PathBuf::from(index_output).join("index.bin")).context("create index file")?);
    let pq_codes = BufWriter::new(
      fs::File::create(PathBuf::from(index_output).join("index.pq-codes.bin")).context("create index file")?,
    );
    let descriptor_codes = BufWriter::new(
      fs::File::create(PathBuf::from(index_output).join("index.descriptor-codes.bin")).context("create index file")?,
    );
  ...
  for batch in &rx.iter().filter_map(initial_filter).chunks(BATCH_SIZE) {
    ...
    if let (Some(read_out_vertices), Some(index_output_file)) = (&mut read_out_vertices, &mut index_output_file) {
      ...
      for (i, (x, _embedding, mut postfilter)) in batch.into_iter().enumerate() {
        ...
        let mut entry_scores =
          scores[(i * score_model.output_channels)..((i + 1) * score_model.output_channels)].to_vec();
