...
use common::{
  resize_for_embed_sync, FrontendInit, IndexHeader, InferenceServerConfig, PackedIndexEntry, QueryRequest, QueryResult,
};
...
lazy_static! {
  ...
  static ref TERMS_COUNTER: IntCounterVec =
    register_int_counter_vec!("mse_terms", "terms used in queries, by type", &["type"]).unwrap();
  ...
  while let Some(pts) = next_several_unvisited(&mut scratch.neighbour_buffer, beamwidth) {
    ...
    for handle in join_handles {
      ...
      if scratch.visited.insert(node.id) && node.url.len() > 0 {
        ...
        scratch
          .visited_embeddings
          .extend(bytemuck::cast_slice(&node.vector).iter().map(|x: &f16| x.to_f32()));
      };
...
async fn evaluate(args: Arc<CLIArguments>, memory_maps: Arc<MemoryMaps>) -> Result<()> {
  ...
  for query_vector in queries.iter() {
    ...
    let selected_shard = index
      ...
      .shards
      .iter()
      .position_max_by_key(|x| scale_dot_result_f64(SpatialSimilarity::dot(&x.0, &query_vector_fp32).unwrap()))
      .unwrap();
  ...
  fn call(&self, req: Request<Incoming>) -> Self::Future {
    ...
    Box::pin(async move {
      let mut body = match (req.method(), req.uri().path()) {
        ...
        (&Method::POST, "/") => {
          ...
          let query = common::get_total_embedding(
            ...
            |batch, _config| query_clip_server(config.clip_server.as_str(), "/", Some(batch)),
          ...
          let selected_shard = index
            ...
            .shards
            .iter()
            .position_max_by_key(|x| scale_dot_result_f64(SpatialSimilarity::dot(&x.0, &query).unwrap()))
            .unwrap();
          ...
          .await
          .map_err(|e| anyhow::anyhow!("threadpool error: {:?}", e))?;
      ...
      body
        ...
        .or_insert(hyper::header::HeaderValue::from_static("GET, POST, OPTIONS"));
      body
        .headers_mut()
...
fn initialize_memory_maps(args: &CLIArguments) -> Result<MemoryMaps> {
  ...
  let descriptors = unsafe { MmapOptions::new().populate().map_copy_read_only(&descriptors_file)? };
