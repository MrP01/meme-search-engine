...
async fn scan_image_chunk(client: &Client, image: &[u8], image_width: u32, image_height: u32) -> Result<ScanResult> {
  ...
  headers.insert(
    "Cookie",
    HeaderValue::from_str(&format!("SOCS=CAESEwgDEgk0ODE3Nzk3MjQaAmVuIAEaBgiA_LyaBg; stcs={}", timestamp))?,
  );
  ...
  let captures = re.captures(&body).ok_or_else(|| anyhow!("invalid API response"))?;
...
pub async fn scan_image(client: &Client, image: &DynamicImage) -> Result<ScanResult> {
  ...
  let (width, height, image) = if width > MAX_DIM {
    ...
    let new_image =
      tokio::task::block_in_place(|| image.resize_exact(MAX_DIM, height, image::imageops::FilterType::CatmullRom));
