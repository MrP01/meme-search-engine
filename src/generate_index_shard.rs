...
use argh::FromArgs;
use diskann::{
    build_graph, medioid, random_fill_graph, report_degrees, robust_stitch, vector::VectorList,
    IndexBuildConfig, IndexGraph, Timer,
};
use half::f16;
...
use common::{ShardHeader, ShardInputHeader, ShardedRecord};
...
struct CLIArguments {
    ...
    #[argh(
        ...
        short = 'L',
        default = "192",
        description = "search list size (higher is better but slower)"
    )]
    l: usize,
    ...
    #[argh(
        ...
        short = 'C',
        default = "750",
        description = "max candidate list size"
    )]
    maxc: usize,
    #[argh(
        option,
        short = 'A',
        default = "65536",
        description = "first pass relaxation factor (times 2^16)"
    )]
    alpha: i64,
    #[argh(
        option,
        short = 'Q',
        default = "65536",
        description = "query set special relaxation factor (times 2^16)"
    )]
    query_alpha: i64,
    #[argh(
        option,
        short = 'B',
        default = "65536",
        description = "second pass relaxation factor (times 2^16)"
    )]
    alpha_2: i64,
...
fn main() -> Result<()> {
...

    {
        ...
        let mut header_f = fs::File::create(&format!(
            "{}/{}.shard-header.msgpack",
            args.out_dir, header.id
        ))?;
