...
model, _, preprocess = open_clip.create_model_and_transforms(
    ...
    pretrained=CONFIG.get(
        "model_path", dict(open_clip.list_pretrained())[CONFIG["model"]]
    ),
    precision="fp16",
)
model.eval()
...
if CONFIG.get("aitemplate_image_models"):
    ...
    conv_weights = (
        state["visual.trunk.patch_embed.proj.weight"]
        .permute((0, 2, 3, 1))
        .contiguous()
        .cuda()
        .half()
                ...
                with inference_time_hist.labels(
                    ...
                    if fast_image_fns:
                        ...
                        features = torch.zeros(
                            (batch, model.text.text_projection.out_features)
                        )
                        while progress < batch:
                            biggest_available = max(
                                x
                                for x in fast_image_fns.keys()
                                if x <= (batch - progress)
                            )
                            chunk = fast_image_fns[biggest_available](
                                images[progress : progress + biggest_available]
                            )
    ...
    while True:
        ...
        try:
            if text:
                ...
                images = torch.stack(
                    [preprocess(Image.open(io.BytesIO(im))).half() for im in images]
                ).to(device)
            else:
...
async def run_inference(request):
    ...
    if results[0]:
        ...
        print(results[1])
    return web.Response(
        body=msgpack.dumps(body_data), status=status, content_type="application/msgpack"
    )


image_size = [t for t in preprocess.transforms if isinstance(t, transforms.Resize)][
    0
].size
