...
def partition_soft(vectors, k, max_iter=100, batch_size=8192):
    ...
    with autograd.detect_anomaly():
        for it in range(max_iter):
            ...
            print(
                temperature,
                size_scale * size_loss.detach().tolist(),
                bias_scale * bias_loss.detach().tolist(),
                score_scale * score_loss.detach().tolist(),
...
def simulated_annealing(vectors, k, max_iter=100, batch_size=31768):
    ...
    def fitness(centroids):
        ...
        return (
            distances_from_ideal_cluster_size.max(),
            distances_from_ideal_cluster_size.argmax(dim=1),
        )

    _global_best, global_best_result = None, 1000000
