...
class Index:
    ...
    def search(self, query, top_k):
        ...
        try:
            ...
            pass
        return [
            {"score": float(distance), "file": self.associated_filenames[index]}
            for index, distance in zip(indices, distances)
        ]

    async def run_ocr(self):
        if not CONFIG.get("enable_ocr"):
            return
        ...
        unocred = await conn.execute_fetchall(
            "SELECT files.filename FROM files LEFT JOIN ocr ON files.filename = ocr.filename WHERE ocr.scan_time IS NULL OR ocr.scan_time < files.modtime"
        )

        ocr_sem = asyncio.Semaphore(
            20
        )  # Google has more concurrency than our internal CLIP backend. I am sure they will be fine.
        load_sem = threading.Semaphore(
            100
        )  # provide backpressure in loading to avoid using 50GB of RAM (this happened)

        async def run_image(filename, chunks):
            try:
                text, regions = await ocr.scan_chunks(self.session, chunks)
                await conn.execute(
                    "INSERT OR REPLACE INTO ocr VALUES (?, ?, ?, ?)",
                    (filename, time.time(), text, json.dumps(regions)),
                )
                await conn.commit()
                sys.stdout.write(".")
                sys.stdout.flush()
                            ...
                            for task in asyncio.as_completed(
                                [
                                    asyncio.get_running_loop().run_in_executor(
                                        ...
                                        load_image,
                                        path,
                                        self.inference_server_config["image_size"],
                                    )
                                    for path in paths
                                ]
                            ):
...
async def main():
    ...
    await index.reload()
    print("Ready")
    if CONFIG.get("no_run_server", False):
        return
...
if __name__ == "__main__":
    ...
    if not CONFIG.get("no_run_server", False):
        loop.run_forever()
