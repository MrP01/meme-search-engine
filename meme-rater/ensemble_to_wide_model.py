...
with torch.inference_mode():
    ...
    for layer in range(config.n_hidden):
        ...
        for i in range(config.n_ensemble):
            big_layer[i * config.d_emb : (i + 1) * config.d_emb] = (
                model.ensemble.models[i].hidden[layer].weight.data.clone()
            )
            big_bias[i * config.d_emb : (i + 1) * config.d_emb] = (
                model.ensemble.models[i].hidden[layer].bias.data.clone()
            )
    ...
    for i in range(10):
        ...
        for layer, bias in zip(out_layers, out_bias):
            r_result = torch.matmul(layer, r_result.T) + bias.unsqueeze(-1).expand(
                config.n_ensemble * config.d_emb, input.shape[0]
            )
    ...
    save_file(
        {"up_proj": out_layers[0], "bias": out_bias[0], "down_proj": downprojection},
        "model.safetensors",
    )
