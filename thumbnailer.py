...
def avif_format(quality):
    ...
    def fn(inpath, outpath):
        if os.path.splitext(inpath)[-1].lower() not in {
            ".jpg",
            ".png",
            ".jpeg",
            ".avif",
...
output_formats = {
    ...
    "jpeg-256k": (
        jpeg_format(target_size=256_000, maxwidth=600, maxheight=600),
        ".jpeg",
        "image/jpeg",
    ),
}
    ...
    for file in os.listdir(directory):
        ...
        if ext in exts:
            ...
            if st.st_mtime != mtime or formats != full_formats:
                ...
                con.execute(
                    "INSERT OR REPLACE INTO thumb VALUES (?, ?, ?)",
                    (rawname, st.st_mtime, generate_output_format_string(formats)),
                )
                con.commit()
                sys.stdout.write(".")
                sys.stdout.flush()
